...
const CONFIG = {
    ...
    pvpRate: 70, // % игр против игроков
    pveRate: 30, // % игр против бота (автоматически 100 - pvpRate)
    ...
    // Награды за матч с ботом (PvE) - только монеты, карт нет
    pveRewards: {
        win: { cards: 0, coins: 50 }, // Половина от PvP
        draw: { cards: 0, coins: 25 }, // Половина от PvP
        loss: { cards: 0, coins: 12 } // Половина от PvP
    },
    
    // Вероятности против бота
    pveWinRate: 80,
    pveDrawRate: 5,
...
class CardGameSimulator {
    ...
    simulateDay(day) {
        ...
        for (let game = 0; game < totalGames; game++) {
            ...
            // Определяем режим игры (PvP или PvE)
            const isPvP = Math.random() * 100 < this.config.pvpRate;
            
            if (isPvP) {
                // PvP игра против другого игрока
                this.stats.pvpGames++;
                ...
                for (let i = 0; i < reward.cards; i++) {
                    const card = this.getCardWithMechanics(true);
                    if (card) {
                ...
                // PvE игра против бота
                this.stats.pveGames++;
                
                const rand = Math.random() * 100;
                const { pveWinRate, pveDrawRate, pveLossRate } = this.config;
                
                let result;
                if (rand < pveWinRate) {
                    result = 'win';
                    this.stats.pveWins++;
                } else if (rand < pveWinRate + pveDrawRate) {
                    result = 'draw';
                    this.stats.pveDraws++;
                } else {
                    result = 'loss';
                    this.stats.pveLosses++;
                }
                
                // Награды за PvE (только монеты, карт нет)
                const reward = this.config.pveRewards[result];
                this.stats.coinsEarned += reward.coins;
                this.coins += reward.coins;
            }
        }
...
function readConfigFromUI() {
    // Читаем PvP/PvE распределение
    const pvpRate = parseFloat(document.getElementById('pvpRate').value);
    const pveRate = 100 - pvpRate; // Автоматически
    
    // Читаем вероятности для PvE
    const pveWinRate = parseFloat(document.getElementById('pveWinRate').value);
    const pveDrawRate = parseFloat(document.getElementById('pveDrawRate').value);
    const pveLossRate = 100 - pveWinRate - pveDrawRate;
    ...
    return {
        ...
        pveDrawRate: pveDrawRate,
        pveLossRate: pveLossRate,
        ...
        // Награды за PvE (только монеты)
        pveRewards: {
            win: {
                cards: 0,
                coins: parseInt(document.getElementById('pveWinCoins').value)
            },
            draw: {
                cards: 0,
                coins: parseInt(document.getElementById('pveDrawCoins').value)
            },
            loss: {
                cards: 0,
                coins: parseInt(document.getElementById('pveLossCoins').value)
            }
        },
...
function displaySingleResult(result) {
    ...
    // PvP/PvE статистика
    document.getElementById('pvpWins').textContent = stats.pvpWins;
    document.getElementById('pvpDraws').textContent = stats.pvpDraws;
    document.getElementById('pvpLosses').textContent = stats.pvpLosses;
    document.getElementById('pveWins').textContent = stats.pveWins;
    document.getElementById('pveDraws').textContent = stats.pveDraws;
    document.getElementById('pveLosses').textContent = stats.pveLosses;
...
function resetConfig() {
    ...
    // PvP/PvE распределение
    document.getElementById('pvpRate').value = 70;
    document.getElementById('pveRate').value = 30;
    ...
    // Награды за PvE
    document.getElementById('pveWinCoins').value = 50;
    document.getElementById('pveDrawCoins').value = 25;
    document.getElementById('pveLossCoins').value = 12;
    ...
    // Вероятности PvE
    document.getElementById('pveWinRate').value = 80;
    document.getElementById('pveDrawRate').value = 5;
    document.getElementById('pveLossRate').value = 15;
